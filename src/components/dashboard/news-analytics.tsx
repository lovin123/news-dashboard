...
export function NewsAnalytics() {
  ...
  const authorData = newsData.reduce((acc, item) => {
  ...
  const typeData = newsData.reduce((acc, item) => {
  ...
  // Prepare source data
  const sourceData = newsData.reduce((acc, item) => {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Visualize news and blog trends by author, type, and source
        </CardDescription>
      </CardHeader>
      <CardContent>
        {error ? (
