...
export function NewsTable() {
  ...
  useEffect(() => {
    ...
    if (fromDate) {
      ...
      filtered = filtered.filter((item) => new Date(item.publishedAt) >= fromDateObj)
    }

    if (toDate) {
      const toDateObj = new Date(toDate)
      filtered = filtered.filter((item) => new Date(item.publishedAt) <= toDateObj)
    }
            ...
            {isLoading ? (
              ...
              paginatedData.map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Badge variant={item.type === "news" ? "default" : "secondary"}>{item.type}</Badge>
                  </TableCell>
              ...
              {Array.from({ length: Math.min(totalPages, 5) }).map((_, i) => {
                ...
                return (
                  <PaginationItem key={i}>
                    <PaginationLink onClick={() => setCurrentPage(pageToShow)} isActive={currentPage === pageToShow}>
                      {pageToShow}
                    </PaginationLink>
                  </PaginationItem>
                )
