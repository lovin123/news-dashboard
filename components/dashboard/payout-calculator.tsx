...
export function PayoutCalculator() {
  ...
  // Calculate totals
  const totalNewsArticles = newsData.filter((item) => item.type === "news").length
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Set payout rates for different content types</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg border p-2">
                ...
                <div className="text-lg font-bold">{isLoading ? "..." : totalNewsArticles}</div>
              </div>
              <div className="rounded-lg border p-2">
                <div className="text-xs text-muted-foreground">Blogs</div>
                <div className="text-lg font-bold">{isLoading ? "..." : totalBlogPosts}</div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="rounded-lg border bg-primary/10 p-2">
              ...
              <div className="text-lg font-bold">{isLoading ? "..." : `$${totalPayout.toFixed(2)}`}</div>
            </div>
          </div>
        </div>
      </CardContent>
